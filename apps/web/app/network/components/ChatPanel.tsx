...
const ChatPanel: React.FC<ChatPanelProps> = ({ isMobile, onBack }) => {
  ...
  const setSidebarTransition = useSetAtom(sidebarTransitionLoadingAtom);
    ...
    <div className="flex flex-col h-full w-full bg-black relative overflow-hidden ml-2">
      <div className="flex-none flex items-center p-3 bg-black border-b border-zinc-800 z-10">
        ...
        <div className="flex-grow min-w-0">
          ...
          {isDM ? (
            <span
              className={`text-xs block truncate ${convoData.isOnline ? "text-green-400" : "text-zinc-500"}`}
            >
              {statusText}
            </span>
          ) : (
      ...
      <AnimatePresence>
        {typingUser && (
          <motion.div
            ...
            className="flex-none px-4 pb-1 text-xs text-zinc-500 italic bg-black"
          >
